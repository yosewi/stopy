...
int main(int argc, char **argv)
{
    ...
    for(int i = 0; i < m; i++)
    {
        ...
        dogs[i].pQuitFlag = &quitFlag;
        dogs[i].pmxQuitFlag = &mxQuitFlag;
        dogs[i].pMask = &newMask;
    }
    ...
    sigArgs.pmxQuitFlag = &mxQuitFlag;
    if(pthread_create(&signalTid, NULL, signal_handling, &sigArgs)){
        ERR("pthread_crate");
    }
